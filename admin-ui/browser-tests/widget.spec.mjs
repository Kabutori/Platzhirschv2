...
const config = { name: 'Restaurant <img src=x onerror=alert(1)>', timezone: 'Europe/Berlin', duration_minutes: 120, accent: '#c08050' };
const cors = { 'Access-Control-Allow-Origin': 'http://127.0.0.1:4174', 'Access-Control-Allow-Methods': 'GET,POST,OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type,Accept' };
...
test('isolates host CSS, treats names as text and submits only one booking', async ({ page }) => {
  ...
  await setup(page, async route => {
    if (route.request().method() === 'GET') return fulfill(route, { tables: [{ id: 1, name: 'Saal', capacity: 4 }] });
...
test('ignores stale availability replies after changing the date', async ({ page }) => {
  await setup(page, async route => {
    ...
    return fulfill(route, { tables: [{ id: old ? 1 : 2, name: old ? 'Alter Termin' : 'Neuer Termin', capacity: 4 }] });
  });
  await page.getByLabel('Datum und Uhrzeit').fill('2030-05-20T18:00');
  await page.getByRole('button', { name: 'Verfügbare Tische anzeigen' }).click();
