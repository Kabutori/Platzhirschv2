...
const admin = {id:1, name:'Owner', role:'restaurant_admin', tenant_id:1, permissions:['reservation.read','reservation.write','reservation.cancel','reservation.export','roles.manage','team.manage','restaurant.configure','restaurant.profile','widget.manage','support.access']};
async function mockApi(page, user = admin) {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (path.endsWith('/csrf')) body = {token:'test-csrf'};
      ...
      if (route.request().method() === 'POST') { saved.push(route.request().postDataJSON()); body = {id:1}; }
      else body = {catalog:{'reservation.read':'Reservierungen und Belegung ansehen','reservation.write':'Reservierungen anlegen und bearbeiten'},roles:[]};
    }
