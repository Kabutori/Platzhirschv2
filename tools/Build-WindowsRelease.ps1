...
New-Item -ItemType Directory -Path "$out\mcp" -Force | Out-Null
Copy-Item "$root\admin-ui\packages\mcp\src\server.mjs" "$out\mcp\server.mjs"
Copy-Item "$root\docs\API-BETRIEB.md" "$out\API-BETRIEB.md"
