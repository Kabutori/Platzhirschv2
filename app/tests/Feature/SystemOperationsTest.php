...
class SystemOperationsTest extends TestCase {
 ...
 private function state(?string $heartbeat=null): void {File::put($this->directory.'/public/state.json',json_encode(['heartbeat'=>$heartbeat??gmdate('c'),'packages'=>[['id'=>'Platzhirsch-0.1.0-preview.103','label'=>'Approved']],'backups'=>[],'jobs'=>[]]));}
 ...
 private function payload(): array {return ['action'=>'update','target'=>'Platzhirsch-0.1.0-preview.103','password'=>'Correct-password-123','confirmation'=>true,'request_id'=>(string)Str::uuid()];}
